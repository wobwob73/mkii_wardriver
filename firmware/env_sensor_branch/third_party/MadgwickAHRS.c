 ...
 *
 * Customization for MKII:
 *   - sample frequency exposed as `madgwick_sample_freq` rather than the
 *     hard-coded 512 Hz from the original.
 *   - inverse square root uses C99 1.0f/sqrtf().
 */
...
volatile float madgwick_sample_freq = 100.0f;
...
void MadgwickAHRSupdate(float gx, float gy, float gz,
    ...
    float dt = 1.0f / madgwick_sample_freq;
