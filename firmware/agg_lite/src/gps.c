...
void gps_init(void) {
    ...
    uart_init(GPS_UART_INST, GPS_BAUD);
    gpio_set_function(GPS_TX_PIN, GPIO_FUNC_UART);
    gpio_set_function(GPS_RX_PIN, GPIO_FUNC_UART);
    uart_set_format(GPS_UART_INST, 8, 1, UART_PARITY_NONE);
    uart_set_fifo_enabled(GPS_UART_INST, true);
...
void gps_push_config(void) {
    /*
     * No-op in v1.0.x. The M10/BN-220-class module is pre-configured once via
     * u-center (NMEA RMC+GGA @ 1 Hz; PPS 1 Hz / rising edge). Driving UBX
     * CFG-VALSET at boot is the v1.1 target (lite_aggregator_v1_0.md §5, open
     * item 6) — the same placeholder + rationale as the STM32 firmware. The
     * fix gate (§10) tolerates a factory-default module.
     */
}
...
/* NMEA-0183 checksum: XOR between '$' (exclusive) and '*' (exclusive). A
 * corrupted byte must not update the timebase, so reject unless '*HH' is
 * exactly the last three bytes and matches (F-003). */
...
void gps_poll(void) {
    while (uart_is_readable(GPS_UART_INST)) {
        uint8_t b = (uint8_t)uart_getc(GPS_UART_INST);
        g_bytes_total++;
        if (b == '\n') {
            if (g_line_pos > 0) {
                g_line[g_line_pos] = '\0';
