...
bool pio_uart_subsys_init(void) {
    ...
    uart_tx_program_init(pio1, (uint)g_tx_sm_pio1, (uint)g_tx_offset_pio1,
                         LEAF_W24_TX_PIN, LEAF_BAUD);
    g_tx_current_pin = LEAF_W24_TX_PIN;
    return true;
}
...
/*
 * Retarget the single shared TX state machine at a new downstream pin. F-001:
 * PINCTRL OUT_BASE/SIDESET_BASE are rebuilt and self-checked — re-pinning the
 * GPIO alone left every Leaf's bytes on the originally configured pin.
 */
