...
// $CF,leaf_id,mode,channel_set_id,dwell_ms,reserved
static void handle_cf(char **f, int n) {
    ...
    ChannelSetId set_id = (ChannelSetId)parse_int(f[3], CSID_UNII1_2A);
    uint16_t dwell_ms = (uint16_t)parse_int(f[4], 0);
    (void)f[5];

    if (!cfg::adopt_from_cf(leaf_id, mode, set_id, dwell_ms)) {
        hb::note_error();
        return;
    }
    switch_to_mode(mode);
...
static void handle_ch(char **f, int n) {
    ...
    uint16_t lo = (uint16_t)parse_int(f[2], 0);
    uint16_t hi = (uint16_t)parse_int(f[3], 0);
    if (!cfg::set_channel_mask_lo_hi(lo, hi)) {
        hb::send_immediate();
        return;
    }
