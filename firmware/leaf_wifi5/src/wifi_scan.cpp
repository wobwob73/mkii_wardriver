...
static uint8_t  g_chan_list[N_CHANNELS_5G];
static uint8_t  g_chan_count = 0;
static uint8_t  g_chan_idx = 0;
static uint32_t g_chan_mask_cached = 0;
...
static void build_channel_list() {
    uint32_t mask = cfg::channel_mask();
    if (mask == g_chan_mask_cached && g_chan_count > 0) return;
    g_chan_mask_cached = mask;
    g_chan_count = 0;
    for (uint8_t b = 0; b < N_CHANNELS_5G; b++) {
        if (mask & (1u << b)) {
            g_chan_list[g_chan_count++] = CHANNELS_5G[b];
        }
    }
    g_chan_idx = 0;
    if (g_chan_count == 0) {
...
static void start_async(uint8_t channel) {
    if (g_scan_active) return;
    uint32_t dwell = cfg::dwell_ms() == 0 ? 200 : cfg::dwell_ms();
    int rc = WiFi.scanNetworks(true /*async*/, true /*show_hidden*/,
                               true /*passive*/, dwell, channel);
...
void process() {
    ...
    if (!g_scan_active) {
        if (g_chan_count == 0) return;
        g_chan_idx = (uint8_t)((g_chan_idx + 1) % g_chan_count);
        start_async(g_chan_list[g_chan_idx]);
    }
}
