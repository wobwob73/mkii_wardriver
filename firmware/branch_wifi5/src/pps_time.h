 ...
 * struct directly — both go through pps_time_snapshot() (readers) or the
...
/* Called from the GPIO IRQ callback on core0. */
void pps_time_isr(unsigned int gpio, uint32_t events);

/* Called from core1 when a $TM arrives. Applies to the last pending edge;
 * a no-op if no edge is currently pending. */
void pps_time_apply_tm(uint32_t epoch_s, bool fix_ok);

/* Atomic snapshot. Returns true; the out fields are always populated. */
